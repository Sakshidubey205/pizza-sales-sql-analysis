...
-- idendify the highest-priced pizza.
...
-- group the orders by daye and calculate the average numberof pizzas ordered per day.

SELECT
    ROUND(AVG(quantity), 0) as avg_pizza_ordred_per_day
FROM
    (SELECT
...
-- determine the top 3 most ordered pizza types based on revenue for each pizza catgegory.
